...
app.post("/mcp", (req: HttpRequest, res: HttpResponse) => handleMcpPostRequest(config, req, res));
app.get("/mcp", missingApiKey);
app.delete("/mcp", missingApiKey);

const listener = app.listen(port, () => {
  console.log(`Serphouse MCP HTTP server listening on http://${host}:${port}${ServiceInfo.MCP_PATH}`);
}) as Server;
